...
def minimax_min_node(board, color, limit, caching = 0):
    ...
    for move in possible_moves:
        ...
        score = minimax_max_node(new_board, myself, limit-1, caching)
...
def minimax_max_node(board, color, limit, caching = 0): #returns highest possible utility
    ...
    for move in possible_moves:   
        ...
        score = minimax_min_node(new_board, myself, limit-1, caching)
        if score[1] > old_score:
            old_score = score[1]
            return_move = move
    if caching == 1:
...
def select_move_minimax(board, color, limit, caching = 0):
    ...
    if caching == 1:
        board_mapping.clear()
    i, j = minimax_max_node(board, color, limit, caching)[0]
    return i,j
...
def alphabeta_min_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    if ordering == 1:
        ...
        sorted_score = sorted(unsorted_score.items(), key=lambda i:i[1])
    ...
    for move in possible_moves:
        new_board = play_move(board, opponent, move[0], move[1])
        if(alpha < beta):
            score = alphabeta_max_node(new_board, myself, alpha, beta, limit-1, caching, ordering)
            if score[1] < old_score:
                old_score = score[1]
                beta = old_score
                return_move = move
